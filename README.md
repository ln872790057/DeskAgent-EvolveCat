...
> 当前项目定位是内部提效工具，不是商业化产品。重点实现桌面 Agent 的产品架构、交互体验和任务执行链路。

## 为什么做这个项目

通用大模型和 Agent 工具已经很强，但在日常工作流里仍然存在几个问题：

- 工具调用、任务状态、失败原因和最终结果之间缺少清晰链路。
- 结果难以沉淀，偏好、历史任务和纠错经验不能稳定复用。
- 通用 Agent 不贴合团队具体工作流，例如调研计划、来源筛选、报告结构、结果归档等。

DeskAgent 的目标是验证一个桌面常驻 Agent 的产品形态：让 Agent 能在本地工作环境中理解任务、调用工具、后台执行、展示进度、沉淀记忆，并通过质量评估持续迭代。一期以团队最高频的调研任务作为首个验证场景。

## 产品定位

DeskAgent 是一个通用桌面 Agent 原型。

当前选择“调研”作为首个验证场景，能完整检验 Agent 的关键能力：
